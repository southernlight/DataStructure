...
Artist *find_artist(char *name); // �ٸ� �ʿ����� �����ϱ� ������ ������Ͽ� �־�� �ʿ䰡 ����.
...
SNode *find_snode(Artist *ptr_artist, char *title)
{
    ...
    while (ptr_snode != NULL && strcmp(ptr_snode->song->title, title) < 0)
        ptr_snode = ptr_snode->next; //���ǿ� �ٸ� �κ�
...
Artist *add_artist(char *name)
{
    ...
    Artist *p = artist_directory[(unsigned char)name[0]]; // first artist start with name[0]
    Artist *q = NULL;
    ...
    if (p == NULL && q == NULL) // add to empty list
...
{
    // find if the artist already exits
    // return NULL if not
    Artist *ptr_artist = find_artist(artist);
    if (ptr_artist == NULL)
...
void insert_node(Artist *ptr_artist, SNode *ptr_snode)
{
    ...
    // 1.empty 2.at the front 3. at the end 4. middle
